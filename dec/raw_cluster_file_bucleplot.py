...
Procesa todos los .nc de `data/`, normaliza el backscatter,
aplica autoencoder + KMeans (k=8) y genera plots de clusters.
Permite reasignar manualmente etiquetas de clusters antes de guardar.
Resultados en `images/`.
"""
...
#  Obtener la lista de archivos en la carpeta data (adaptado a granada)
...
for nc_file in files:
    ...
    pixel_labels = np.vectorize(lambda x: category_mapping.get(x, 1))(pixel_labels)
   #####    #####    #####    #####    #####    #####    #####    #####    #####    #####    #####    #####  
    ...
    output_path = f"{output_folder}/{nc_file.replace('.nc', '')}_plot.png"
